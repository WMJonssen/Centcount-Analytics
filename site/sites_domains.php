...
* module: Centcount Analytics Free Manage Domains PHP Code *
* version: 1.00 Free *
* author: WM Jonssen *
* date: 03/12/2018 *
...
define('HTML_LOADING', '
		...
		"<td class=\'tdhlt\'  style=\'width:25%\'>'. $GLOBALS['indicator']['Domain'] .'</td>"+
		"<td class=\'tdhlt\'  style=\'width:15%\'>'. $GLOBALS['indicator']['Site Name'] .'</td>"+
		"<td class=\'tdhmid\' style=\'width:15%\'>'. $GLOBALS['indicator']['Site ID'] .'</td>"+
		"<td class=\'tdhmid\' style=\'width:15%\'>'. $GLOBALS['indicator']['Create Time'] .'</td>"+
		"<td class=\'tdhmid\' style=\'width:25%\' colspan=\'3\'>'. $GLOBALS['indicator']['Settings'] .'</td>"+
		"</tr>"+
		"<tr class=\'tra\'>"+
		"<td class=\'tdmid\' colspan=\'8\'><img class=\'mid\' src=\'images/loading.gif\'/>&nbsp;Processing...</td>"+
		"</tr>"+
		"<tr class=\'trfoot\'>"+
		"<td class=\'tdrt\' colspan=\'8\'>"+
		"</td>"+	
		"</tr>"+
		"</table>"
');
...
define('HTML_LOAD_ERROR', '
		...
		"<tr class=\'tra\'>"+
		"<td class=\'tdmid\' colspan=\'8\'>Request timed out! Please refresh</td>"+
		"</tr>"+
...
<div id="frametable" class="frametable">
	...
	if ($i)	{
		...
		$sum = 1;
		for ($row = 0; $row < $i; $row++) {
			if ($GLOBALS['DOMAINS'][$row]['DomainType'] == 1) {
	...
	if ($sum == 1) {
		$tabletext .= '
		<tr class="tra">
			<td class="tdmid" colspan="8">No Data</td>
		</tr>
		';
	}
	...
	<div id="frameinput" class="frameinput">		
		<div class="framebox">
			<div class="inputbox">
				<div class="inputtitle">'. $GLOBALS['indicator']['Add Domain'] .'</div>
				...
				<form name="blockitemform" method="POST" action="" style="width:100%; height:100%; text-align:left; float:left;">
					<div style="width:100%; height:auto; padding:15px; padding-top:0px; text-align:left; float:left;">
						<p style="margin:0px; padding:0px; font-size:13px; height:30px; line-height:30px;">'. $GLOBALS['indicator']['Domain'] .' :</p>
...
<script type="text/javascript">
	...
	function modifyblockitem(x,y) {
		var str=prompt("Modify Domain",y);
		str=str.toLowerCase();
		if (str == y) {
			alert("No changes");
		} else if (str) {
	...
	function deleteblockitem(x,y) {
		if (confirm("Are you sure to delete the domain: " + y + "?")) {
	...
	function addblockitem() {
		var def="Please input new domain here";
		var str=prompt("Add Domain",def);
		if (str == def) {
			alert("No domain added");
		} else if (str) {
			str=str.toLowerCase();
