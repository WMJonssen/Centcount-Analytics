...
* module: Centcount Analytics Free Manage Blocked IPs PHP Code *
* version: 1.00 Free *
* author: WM Jonssen *
* date: 03/12/2018 *
...
function status_html($status) {
	...
	"<table>"+
		...
		"<td class=\'tdhlt\'  style=\'width:20%\'>'. $GLOBALS['indicator']['Blocked IP'] .'</td>"+
		...
		"</tr>"+
		"<tr class=\'tra\'>"+
		"<td class=\'tdmid\' colspan=\'10\'>'. ($status ? 'Request timed out! Please refresh' : '<img class=\'mid\' src=\'images/loading.gif\'/>&nbsp;Processing...') .'</td>"+
		"</tr>"+
		"<tr class=\'trfoot\'>"+
		"<td class=\'tdmid\'>Local IP:</td>"+
		"<td class=\'tdltnoborder\' id=\'ip\'></td>"+
		"<td class=\'tdrtnoborder\' colspan=\'8\'>"+
		"</td>"+	
		"</tr>"+
	"</table>"';
}
...
function blocked_ips_operation() {
		...
		$q = $v . 'q=set domain&type=5&sid=' . $GLOBALS['SITEID'] . '&tz=' . $tz . '&param=' . $param . '&key=' . $key . '&value=' . $value;
...
function blocked_ips_table_html() {
...
<div id="frametable" class="frametable">
	...
	if ($i)	{
		...
		for ($row = 0; $row < $i; $row++) {
			...
			if ($GLOBALS['DOMAINS'][$row]['DomainType'] == 5) {
	...
	$tabletext .= '
		<tr class="trfoot">
			<td class="tdmid" title="Local IP">IP:</td>
			<td class="tdltnoborder" id="ip">'.get_ip().'</td>
			<td class="tdrtnoborder" colspan="8">
	...
	<div id="frameinput" class="frameinput">		
		<div class="framebox">
			<div class="inputbox">
				<div class="inputtitle">'. $GLOBALS['indicator']['Add Blocked IP'] .'</div>
				...
				<form name="blockitemform" method="POST" action="" style="width:100%; height:100%; text-align:left; float:left;">
					<div style="width:100%; height:auto; padding:15px; padding-top:0px; text-align:left; float:left;">
						<p style="margin:0px; padding:0px; font-size:13px; height:30px; line-height:30px;">'. $GLOBALS['indicator']['Blocked IP'] .' :</p>
...
function blocked_ips_html() {

	//$GLOBALS['TEXT_SUBMENU'] = SUBMENU_MANAGE_SITES;
			
	$GLOBALS['TEXT_BODY'] = blocked_ips_table_html($GLOBALS['SITES'], $GLOBALS['SITEID'], $GLOBALS['DOMAINS']);
...
<script type="text/javascript">
	...
	function modifyblockitem(x,y) {
		var str=prompt("Modify Blocked IP",y);
		//str=str.toLowerCase();
		if (str == y) {
			alert("No changes");
		} else if (str) {
	...
	function deleteblockitem(x,y) {
		if (confirm("Are you sure to delete the blocked IP: " + y + "?")) {
	...
	function addblockitem() {
		var def="Please input new IP here";
		var str=prompt("Add Blocked IP",def);
		if (str == def) {
			alert("No blocked IP added");
		} else if (str) {
			//str=str.toLowerCase();
