...
* module: Centcount Analytics Free Manage Blocked Sites PHP Code *
* version: 1.00 Free *
* author: WM Jonssen *
* date: 03/12/2018 *
...
define('HTML_LOADING', '
		...
		"<td class=\'tdhlt\'  style=\'width:20%\'>'. $GLOBALS['indicator']['Blocked Site'] .'</td>"+
...
function blocked_sites_operation() {
		...
		$q = $v . 'q=set domain&type=3&sid=' . $GLOBALS['SITEID'] . '&tz=' . $tz . '&param=' . $param . '&key=' . $key . '&value=' . $value;
...
function blocked_sites_table_html() {
...
<div id="frametable" class="frametable">
	...
	if ($i)	{
		...
		$sum = 1;
		for ($row = 0; $row < $i; $row++) {
			if ($GLOBALS['DOMAINS'][$row]['DomainType'] == 3) {
	...
	<div id="frameinput" class="frameinput">		
		<div class="framebox">
			<div class="inputbox">
				<div class="inputtitle">'. $GLOBALS['indicator']['Add Blocked Site'] .'</div>
				...
				<form name="blockitemform" method="POST" action="" style="width:100%; height:100%; text-align:left; float:left;">
					<div style="width:100%; height:auto; padding:15px; padding-top:0px; text-align:left; float:left;">
						<p style="margin:0px; padding:0px; font-size:13px; height:30px; line-height:30px;">'. $GLOBALS['indicator']['Blocked Site'] .' :</p>
...
function blocked_sites_html() {

	//$GLOBALS['TEXT_SUBMENU'] = SUBMENU_MANAGE_SITES;
			
	$GLOBALS['TEXT_BODY'] = blocked_sites_table_html($GLOBALS['SITES'], $GLOBALS['SITEID'], $GLOBALS['DOMAINS']);
...
table{width:100%; min-width:960px; border:#ccc 1px solid; border-collapse:collapse; border-spacing:0; font-family:"Microsoft Yahei",Arial,Verdana;}
...
.tdlt{text-align:left; table-layout:fixed;}
...
<script type="text/javascript">
	...
	function modifyblockitem(x,y) {
		var str=prompt("Modify Blocked Site",y);
		//str=str.toLowerCase();
		if (str == y) {
			alert("No changes");
		} else if (str) {
	...
	function deleteblockitem(x,y) {
		if (confirm("Are you sure to delete the blocked site: " + y + "?")) {
	...
	function addblockitem() {
		var def="Please input new site url here";
		var str=prompt("Add Blocked Site",def);
		if (str == def) {
			alert("No blocked site added");
		} else if (str) {
			//str=str.toLowerCase();
